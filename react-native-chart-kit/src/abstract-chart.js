...
class AbstractChart extends Component {
  ...
  // 
  // RIO: These are the lables on the Y axis
  // 
  renderVerticalLabels = config => {
    const { labels = [], width, height, paddingRight, paddingTop, horizontalOffset = 0 } = config
    const fontSize = 12
    const barHeight = (height-paddingTop)/(labels.length + 2);
    return labels.map((label, i) => {
      return (

        <Text style={{textAlign: 'left'}}
          key={Math.random()}
          x= {paddingRight} //{((width - paddingRight) / labels.length * (i)) + paddingRight + horizontalOffset}
          width = {200}
          y={(paddingTop + (i * (barHeight )) + barHeight)}     
          fontSize={fontSize}
          // fill={this.props.chartConfig.color(0.5)}
          textAnchor="end"
          fill = 'yellow'
